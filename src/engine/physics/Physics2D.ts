...
function overlaps(a: { minX: number; minY: number; maxX: number; maxY: number }, b: { minX: number; minY: number; maxX: number; maxY: number }): boolean {
  ...
  step(objects: Iterable<GameObject>): void {
    ...
    for (const go of objects) {
      ...
      for (const c of go.getComponents()) {
        ...
        if (c instanceof Collider2D) colliders.push(c)
      }
    }
    ...
    for (let i = 0; i < colliders.length; i++) {
      ...
      for (let j = i + 1; j < colliders.length; j++) {
        ...
        if (!goB || goA === goB) continue

        if (!overlaps(aabbA, b.getWorldAabb())) continue
    ...
    this._previousPairs.clear()
    for (const k of currentPairs) this._previousPairs.add(k)
  }
}
