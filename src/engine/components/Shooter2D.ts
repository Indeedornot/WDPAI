...
export class Shooter2D extends Component {
  ...
  update(dt: number): void {
    ...
    for (let i = 0; i < shots; i++) {
      ...
      projectile.addComponent(new SpriteRenderer2D({ size: this.projectileSize, color: this.projectileColor }))
      ...
      const dmg = projectile.addComponent(new DamageOnCollision2D({ damage: this.projectileDamage, victimTag: this.projectileVictimTag, oncePerContact: true }))
      ...
      projectile.addComponent(new Lifetime({ seconds: this.projectileLifetimeSeconds, expireMode: this.projectileExpireMode }))
      ...
      if (this.projectileExpireMode === 'freeze' && this.projectileStickyTotalLifetimeSeconds > 0) {
        projectile.addComponent(new Lifetime({ seconds: this.projectileStickyTotalLifetimeSeconds, expireMode: 'destroy' }))
      }
