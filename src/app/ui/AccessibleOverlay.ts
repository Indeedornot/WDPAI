...
export class AccessibleOverlay {
  ...
  private render(): void {
    ...
    const hpText = playerRow?.hp ? `${Math.round(playerRow.hp.current)}/${Math.round(playerRow.hp.max)}` : 'n/a'
    ...
    for (const r of top) {
      ...
      const dist = r.distanceToPlayer != null ? `· ${Math.round(r.distanceToPlayer)} units away` : ''
