...
/** Parses untrusted (e.g. localStorage) data into a valid ControlsConfig. */
export class ControlsConfigParser
{
  ...
  static parse(raw: unknown): ControlsConfig
  {
    ...
    if (!ControlsConfigParser.isRecord(raw))
    {
      return base;
    }

    const movementRaw = raw['movement'];
    if (ControlsConfigParser.isRecord(movementRaw))
    {
      for (const k of Object.keys(base.movement))
      {
        ...
        if (typeof v === 'string')
        {
          (base.movement as Record<string, string>)[k] = v;
        }
      }
    }

    const aimRaw = raw['aim'];
    if (ControlsConfigParser.isRecord(aimRaw))
    {
      for (const k of Object.keys(base.aim))
      {
        ...
        if (typeof v === 'string')
        {
          (base.aim as Record<string, string>)[k] = v;
        }
      }
    }

    base.shootKey = ControlsConfigParser.readString(raw['shootKey'], base.shootKey);
    return base;
  }
}
